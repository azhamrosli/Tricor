Public Class frmNote_Movement
    Dim clsNote As clsNote_Movement = Nothing
    ...
    Public MM_ID As Integer = 0
    Public Data_SubID As Integer = 0
    Public Type_Movement As Integer = 0
    Public Type_Addless As Integer = 0
    Public RowDescription As String = Nothing
    Public DsMovement As DataSet = Nothing
    ...
    '                DsMovement.Tables("MOVEMENT_NOTE").ImportRow(rowx)
    '            Next

    '        End If

    '        PROFITLOSSACCOUNTNOTEBindingSource.DataSource = DsMovement.Tables("MOVEMENT_NOTE")
    '    Catch ex As Exception
    ...
    Private Sub LoadData()
        Try
            ...
            txtMM_ID.EditValue = MM_ID
            txtData_SubID.EditValue = Data_SubID
            txtDescription.EditValue = RowDescription
            txtTagID.EditValue = TagID
            ...
            If Type_Movement = 0 Then
                ...
                If Type_Addless = 0 Then
                    'Add
                    dtValidate = clsMoveNormal.Load_MovementNormal_Add(Data_SubID, MM_ID)
                Else
                    'Less
                    dtValidate = clsMoveNormal.Load_MovementNormal_Deduct(Data_SubID, MM_ID)
                End If
            Else
                'Complex
                If Type_Addless = 0 Then
                    'Add
                    dtValidate = ADO.Load_MovementComplex_Add(Data_SubID, MM_ID)
                Else
                    'Less
                    dtValidate = ADO.Load_MovementComplex_Deduct(Data_SubID, MM_ID)
                End If
            End If
            ...
            dt = clsNote.Load_Note(TagID, MM_ID, Type_Movement, Type_Addless, ErrorLog)
            ...
            If isEdit Then
                ...
                DsMovement.Tables("MOVEMENT_NOTE").ImportRow(dt.Rows(0))
            ...
            Else
                ...
                dtRow = DsMovement.Tables("MOVEMENT_NOTE").NewRow
                dtRow("Title") = "TEST"
                dtRow("ID") = ID
                dtRow("RefNo") = RefNo
                dtRow("YA") = YA
                dtRow("Data_SubID") = Data_SubID
                dtRow("MM_ID") = MM_ID
                dtRow("Type_Movement") = Type_Movement
                dtRow("Type_Addless") = Type_Addless
                dtRow("TypeNote") = 1
                dtRow("Note") = ""
                dtRow("TagID") = TagID
                DsMovement.Tables("MOVEMENT_NOTE").Rows.Add(dtRow)
            End If
            MOVEMENTNOTEATTACHMENTBindingSource.DataSource = DsMovement.Tables("MOVEMENT_NOTE_ATTACHMENT")
            MOVEMENTNOTECOLUMNBindingSource.DataSource = DsMovement.Tables("MOVEMENT_NOTE_COLUMN")
    ...
    Private Sub SimpleButton1_Click(sender As Object, e As EventArgs) Handles SimpleButton1.Click
        Try
            ...
            If rslt = Windows.Forms.DialogResult.Yes Then
                ...
                DsMovement.Tables("MOVEMENT_NOTE_ATTACHMENT").AcceptChanges()
            End If
        Catch ex As Exception
            Dim st As New StackTrace(True)
            st = New StackTrace(ex, True)
        ...
        Try
            If isValid() Then
                ...
                If RGTypeNote.SelectedIndex = 1 Then
                    If DsMovement.Tables("MOVEMENT_NOTE_COLUMN").Rows.Count > 0 Then
                        For Each rowx As DataRow In DsMovement.Tables("MOVEMENT_NOTE_COLUMN").Rows
                ...
                If isEdit Then
                    If clsNote.Update_Note(ID, txtTitleFrontNote.EditValue, txtTitleNote.EditValue, RefNo, YA, MM_ID, Data_SubID, _
                                           Type_Movement, RGTypeNote.SelectedIndex, Type_Addless, txtMemo.EditValue, _
                                            DsMovement.Tables("MOVEMENT_NOTE_ATTACHMENT"), DsMovement.Tables("MOVEMENT_NOTE_COLUMN"), TagID, ErrorLog) Then
                ...
                Else
                    ...
                    If clsNote.Save_Note(txtTitleFrontNote.EditValue, txtTitleNote.EditValue, RefNo, YA, MM_ID, Data_SubID, _
                                         Type_Movement, RGTypeNote.SelectedIndex, Type_Addless, txtMemo.EditValue, DsMovement.Tables("MOVEMENT_NOTE_ATTACHMENT"), _
                                         DsMovement.Tables("MOVEMENT_NOTE_COLUMN"), TagID, tmpID, ErrorLog) Then
        ...
        Try
            ...
            Select Case RGTypeNote.SelectedIndex
                Case 0
                    ...
                    End If
                Case 2
                    If DsMovement.Tables("MOVEMENT_NOTE_ATTACHMENT").Rows.Count = 0 Then
    ...
    Private Sub GridView2_InitNewRow(sender As Object, e As DevExpress.XtraGrid.Views.Grid.InitNewRowEventArgs) Handles GridView2.InitNewRow
        Try
            GridView2.GetDataRow(e.RowHandle)("ParentID") = DsMovement.Tables("MOVEMENT_NOTE").Rows(0)("ID")
        Catch ex As Exception
            Dim st As New StackTrace(True)
             st = New StackTrace(ex, True)
    ...
    Private Sub GridView3_InitNewRow(sender As Object, e As DevExpress.XtraGrid.Views.Grid.InitNewRowEventArgs) Handles GridView3.InitNewRow
        Try
            GridView3.GetDataRow(e.RowHandle)("ParentID") = DsMovement.Tables("MOVEMENT_NOTE").Rows(0)("ID")
        Catch ex As Exception
            Dim st As New StackTrace(True)
             st = New StackTrace(ex, True)
            ...
            If rslt = Windows.Forms.DialogResult.OK Then
                ...
                For Each files As String In OpenFileDialog1.FileNames
                    If files IsNot Nothing AndAlso System.IO.File.Exists(files) Then
                        ...
                        dtRow = DsMovement.Tables("MOVEMENT_NOTE_ATTACHMENT").NewRow
                        dtRow("ParentID") = ID
                        dtRow("Attachment") = byteData
                        dtRow("Extension") = System.IO.Path.GetExtension(files)

                        DsMovement.Tables("MOVEMENT_NOTE_ATTACHMENT").Rows.Add(dtRow)
                    End If
                Next
