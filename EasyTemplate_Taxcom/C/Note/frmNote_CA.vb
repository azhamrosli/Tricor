Public Class frmNote_CA
    ...
    Public CA_ID As Integer = 0
    Public Type_CA As Integer = 0
    ...
    Private Sub LoadData()
        Try
            ...
            '        'Add
            '        dtValidate = clsMoveNormal.Load_MovementNormal_Add(Data_SubID, CA_ID)
            '    Else
            '        'Less
            '        dtValidate = clsMoveNormal.Load_MovementNormal_Deduct(Data_SubID, CA_ID)
            '    End If
            'Else
            '    'Complex
            '    If Type_Addless = 0 Then
            '        'Add
            '        dtValidate = ADO.Load_MovementComplex_Add(Data_SubID, CA_ID)
            '    Else
            '        'Less
            '        dtValidate = ADO.Load_MovementComplex_Deduct(Data_SubID, CA_ID)
            '    End If
            'End If
            ...
            If Type_CA = 2 Then
                dt = clsNote.Load_Note(CA_ID, Type_CA, TagID, ErrorLog)
            Else
                dt = clsNote.Load_Note(CA_ID, Type_CA, ErrorLog)
            End If
            ...
            If isEdit Then
                ...
                dsCA.Tables("CA_NOTE").ImportRow(dt.Rows(0))
                    ...
                    Case 1
                        ...
                        If dtChild IsNot Nothing Then
                            For Each rowx As DataRow In dtChild.Rows
                                dsCA.Tables("CA_NOTE_ATTACHMENT").ImportRow(rowx)
                            Next
                        End If
                    Case 2
                        ...
                        If dtChild IsNot Nothing Then
                            ...
                            For Each rowx As DataRow In dtChild.Rows
                                dsCA.Tables("CA_NOTE_COLUMN").ImportRow(rowx)
                            Next
                        End If
                End Select
            ...
            Else
                ...
                dtRow = dsCA.Tables("CA_NOTE").NewRow
                dtRow("Title") = "TEST"
                dtRow("ID") = ID
                dtRow("RefNo") = RefNo
                dtRow("YA") = YA
                dtRow("CA_ID") = CA_ID
                dtRow("Type_CA") = Type_CA
                dtRow("TypeNote") = 1
                dtRow("Note") = ""
                dtRow("TagID") = TagID
                dsCA.Tables("CA_NOTE").Rows.Add(dtRow)
            End If
            CANOTEATTACHMENTBindingSource.DataSource = dsCA.Tables("CA_NOTE_ATTACHMENT")
            CANOTECOLUMNBindingSource.DataSource = dsCA.Tables("CA_NOTE_COLUMN")
    ...
    Private Sub SimpleButton1_Click(sender As Object, e As EventArgs) Handles SimpleButton1.Click
        Try
            ...
            If rslt = Windows.Forms.DialogResult.Yes Then
                ...
                dsCA.Tables("CA_NOTE_ATTACHMENT").AcceptChanges()
            End If
        Catch ex As Exception
            Dim st As New StackTrace(True)
            st = New StackTrace(ex, True)
        ...
        Try
            If isValid() Then
                ...
                If RGTypeNote.SelectedIndex = 2 Then
                    If dsCA.Tables("CA_NOTE_COLUMN").Rows.Count > 0 Then
                        For Each rowx As DataRow In dsCA.Tables("CA_NOTE_COLUMN").Rows
                ...
                If isEdit Then
                    If clsNote.Update_Note(ID, txtTitleFrontNote.EditValue, txtTitleNote.EditValue, RefNo, YA, CA_ID, RGTypeNote.SelectedIndex, Type_CA, txtMemo.EditValue, _
                                           DsCA.Tables("CA_NOTE_ATTACHMENT"), DsCA.Tables("CA_NOTE_COLUMN"), TagID, ErrorLog) Then
                        MsgBox("Successfully updated data.", MsgBoxStyle.Information)
                    Else
                        MsgBox("Unsuccesfully update data.", MsgBoxStyle.Critical)
                    End If
                    'If clsNote.Update_Note(ID, txtTitleNote.EditValue, RefNo, YA, CA_ID, Data_SubID, _
                    '                       Type_CA, RGTypeNote.SelectedIndex, Type_Addless, txtMemo.EditValue, _
                    '                        dsCA.Tables("CA_NOTE_ATTACHMENT"), dsCA.Tables("CA_NOTE_COLUMN"), TagID, ErrorLog) Then
                    '    MsgBox("Successfully updated data.", MsgBoxStyle.Information)
                    'Else
                    '    MsgBox("Unsuccesfully update data.", MsgBoxStyle.Critical)
                ...
                Else
                    ...
                    If clsNote.Save_Note(txtTitleFrontNote.EditValue, txtTitleNote.EditValue, RefNo, YA, CA_ID, RGTypeNote.SelectedIndex, Type_CA, txtMemo.EditValue, _
                                          DsCA.Tables("CA_NOTE_ATTACHMENT"), DsCA.Tables("CA_NOTE_COLUMN"), TagID, tmpID, ErrorLog) Then
                        isEdit = True
                        ID = tmpID
                        MsgBox("Successfully saved data.", MsgBoxStyle.Information)
                    Else
                        MsgBox("Unsuccesfully save data.", MsgBoxStyle.Critical)

                    End If
                    'If clsNote.Save_Note(txtTitleNote.EditValue, RefNo, YA, CA_ID, Data_SubID, _
                    '                     Type_CA, RGTypeNote.SelectedIndex, Type_Addless, txtMemo.EditValue, dsCA.Tables("CA_NOTE_ATTACHMENT"), _
                    '                     dsCA.Tables("CA_NOTE_COLUMN"), TagID, tmpID, ErrorLog) Then
        ...
        Try
            ...
            Select Case RGTypeNote.SelectedIndex
                Case 0
                    ...
                    End If
                Case 1
                    If dsCA.Tables("CA_NOTE_ATTACHMENT").Rows.Count = 0 Then
    ...
    Private Sub GridView2_InitNewRow(sender As Object, e As DevExpress.XtraGrid.Views.Grid.InitNewRowEventArgs) Handles GridView2.InitNewRow
        Try
            GridView2.GetDataRow(e.RowHandle)("ParentID") = dsCA.Tables("CA_NOTE").Rows(0)("ID")
        Catch ex As Exception
            Dim st As New StackTrace(True)
            st = New StackTrace(ex, True)
    ...
    Private Sub GridView3_InitNewRow(sender As Object, e As DevExpress.XtraGrid.Views.Grid.InitNewRowEventArgs) Handles GridView3.InitNewRow
        Try
            GridView3.GetDataRow(e.RowHandle)("ParentID") = DsCA.Tables("CA_NOTE").Rows(0)("ID")
        Catch ex As Exception
            Dim st As New StackTrace(True)
            st = New StackTrace(ex, True)
            ...
            If rslt = Windows.Forms.DialogResult.OK Then
                ...
                For Each files As String In OpenFileDialog1.FileNames
                    If files IsNot Nothing AndAlso System.IO.File.Exists(files) Then
                        ...
                        dtRow = dsCA.Tables("CA_NOTE_ATTACHMENT").NewRow
                        dtRow("ParentID") = ID
                        dtRow("Attachment") = byteData
                        dtRow("Extension") = System.IO.Path.GetExtension(files)

                        dsCA.Tables("CA_NOTE_ATTACHMENT").Rows.Add(dtRow)
                    End If
                Next
