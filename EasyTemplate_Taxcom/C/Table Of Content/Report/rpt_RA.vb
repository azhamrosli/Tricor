Public Class rpt_RA
    ...
    Private Sub lbl_WithDrawal_Amount_TextChanged(sender As Object, e As EventArgs) Handles lbl_WithDrawal_Amount.TextChanged
        Try
            ...
            If Total_WithDrawal > 0 Then
                lblRMLeft.Visible = True
            End If
        Catch ex As Exception
            Dim st As New StackTrace(True)
             st = New StackTrace(ex, True)
    ...
    Private Sub XrLabel24_BeforePrint(sender As Object, e As Printing.PrintEventArgs) Handles XrLabel24.BeforePrint
        Try
            Dim row As DataRow = DetailReport2.GetCurrentRow()

            If row IsNot Nothing Then
                If IsDBNull(row("RAAADJ_ITEM")) = False AndAlso CDec(row("RAAADJ_ITEM")) > 0 Then
                    XrLabel24.Text = "ADD:"
                Else
                    XrLabel24.Text = "LESS:"
                End If
