...
Public Class ucPNL_p4DrawingPLStock
    ...
    Public Property DataView_Main() As dsPNL
        Get
            Return clsPNL_.ds_p4DrawingPLStock
        End Get
        Set(ByVal value As dsPNL)
            clsPNL_.ds_p4DrawingPLStock = value
        End Set
    End Property
 ...
 st = New StackTrace(ex, True)
    ...
    Public Sub LoadData(Optional ByRef Errorlog As clsError = Nothing)
        Try
            ...
            PLEXPNONALLOWEXPENDBindingSource.DataSource = clsPNL_.ds_p4DrawingPLStock.Tables(dtParent("TableName"))
...
Dim st As New StackTrace(True)
 st = New StackTrace(ex, True)
    ...
    Private Sub btnMoveUp_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles btnMoveUp.ItemClick
        Try
            clsPNL_.MoveItemsInListView(True, dtParent, RefNo.EditValue, GridView1, clsPNL_.ds_p4DrawingPLStock, ErrorLog)
        Catch ex As Exception
Dim st As New StackTrace(True)
 st = New StackTrace(ex, True)
    ...
    Private Sub btnMoveDown_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles btnMoveDown.ItemClick
        Try
            clsPNL_.MoveItemsInListView(False, dtParent, RefNo.EditValue, GridView1, clsPNL_.ds_p4DrawingPLStock, ErrorLog)
        Catch ex As Exception
Dim st As New StackTrace(True)
 st = New StackTrace(ex, True)
...
Dim st As New StackTrace(True)
 st = New StackTrace(ex, True)
    ...
    Private Sub UpdateCalculation()
        Try
            Try
                If clsPNL_.reCalc_SubTotalView(dtParent, clsPNL_.ds_p4DrawingPLStock, ErrorLog) = False Then

                    MsgBox("Failed to Update." & vbCrLf & ErrorLog.ErrorName & vbCrLf & ErrorLog.ErrorMessage, MsgBoxStyle.Critical)
                Else
                    clsPNL_.CalcTotalofView(dtParent, clsPNL_.ds_p4DrawingPLStock, 0, ErrorLog)
                End If
            Catch ex As Exception
Dim st As New StackTrace(True)
 st = New StackTrace(ex, True)
 ...
 st = New StackTrace(ex, True)
    ...
    Private Sub GridView1_ShowingEditor(sender As Object, e As System.ComponentModel.CancelEventArgs) Handles GridView1.ShowingEditor
        Try
            If TypeOf sender Is GridView Then
                Dim view As GridView = CType(sender, GridView)
                e.Cancel = clsPNL_.DisableAmountIfGotChild(dtParent, view, clsPNL_.ds_p4DrawingPLStock, ErrorLog)
            End If
    ...
    Private Sub UpdateCalculation_Details()
        Try
            If clsPNL_.reCalc_SubTotalView(dtParent, clsPNL_.ds_p4DrawingPLStock, ErrorLog) = False Then
                ...
                MsgBox("Failed to Update." & vbCrLf & ErrorLog.ErrorName & vbCrLf & ErrorLog.ErrorMessage, MsgBoxStyle.Critical)
            Else
                If clsPNL_.MappingSourceNo(dtParent, clsPNL_.ds_p4DrawingPLStock, ErrorLog) = False Then
                    MsgBox("Failed to update." & vbCrLf & ErrorLog.ErrorName & vbCrLf & ErrorLog.ErrorMessage, MsgBoxStyle.Critical)
                Else
                    clsPNL_.CalcTotalofView(dtParent, clsPNL_.ds_p4DrawingPLStock, 0, ErrorLog)
                End If
