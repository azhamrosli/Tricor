    ...
    Private Sub LoadData(Optional Type As Integer = 0)
        Try
            ...
            For i As Integer = 0 To dt.Rows.Count - 1
                clsPNL_.dsDataset.Tables("PROFIT_LOSS_ACCOUNT").ImportRow(dt.Rows(i))
            Next

            PROFITLOSSACCOUNTBindingSource.DataSource = clsPNL_.dsDataset.Tables("PROFIT_LOSS_ACCOUNT")
        Catch ex As Exception
Dim st As New StackTrace(True)
 st = New StackTrace(ex, True)
...
Dim st As New StackTrace(True)
 st = New StackTrace(ex, True)
    ...
    Private Sub cboYA_SelectedIndexChanged(sender As Object, e As EventArgs)
        Try
            LoadData(2)
        Catch ex As Exception
Dim st As New StackTrace(True)
...
Dim st As New StackTrace(True)
 st = New StackTrace(ex, True)
        ...
        End Try
    End Sub
    Private Sub btnPrint_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs)
        'Try
